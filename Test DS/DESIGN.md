  ...
  system:
    color:
      ...
      dark:
        ...
        frostedStrong: "rgba(156, 156, 159, 0.302)"
        ...
        backdrop: "rgba(36, 36, 40, 0.5)"
    typescale:
      fontFamily: "\"Porsche Next\", \"Apple SD Gothic Neo\", \"Malgun Gothic\", \"Noto Sans KR\", \"Arial Narrow\", Arial, sans-serif"
      weights:
        normal: 400
        semibold: 600
        bold: 700
...
- **Frosted Glass Elevation:** Porsche interfaces avoid artificial multi-layered drop shadows. Instead, surfaces achieve depth and spatial hierarchy through **frosted glass overlays** (`backdrop-filter: blur(32px)`) with variable alpha opacity (`frosted-soft`, `frosted`, `frosted-strong`), mirroring lightweight automotive materials.
...
- **Fluid Typography System:** Text scales dynamically using mathematical `clamp()` functions across viewport widths rather than abrupt media query jumps, guaranteeing optimal legibility from mobile cockpits to ultra-wide desktop monitors.
- **8px Baseline Quantum Grid:** All spatial relationships (padding, margins, gaps) adhere to an 8px quantum scale with 4px half-steps for micro-alignments.
...
- **Secondary Action Fill:** Uses `sys.color.frostedStrong` (`rgba(100, 101, 114, 0.236)` in Light, `rgba(156, 156, 159, 0.302)` in Dark) with `sys.color.primary` text.
...
Font Stack: `"Porsche Next", "Apple SD Gothic Neo", "Malgun Gothic", "Noto Sans KR", "Arial Narrow", Arial, sans-serif`

| Tier | Size Calculation | Weight | Typical Usage |
| :--- | :--- | :--- | :--- |
| `5xl` | `clamp(2.28rem, 5.2vw + 1.24rem, 7.48rem)` | 700 Bold | Massive hero stage title |
| `4xl` | `clamp(2.03rem, 3.58vw + 1.31rem, 5.61rem)` | 700 Bold | Screen display hero |
...
*Always use registered `<p-*>` web components. Never substitute raw unstyled HTML buttons or inputs.*

### 1. Actions
- `<p-button variant="primary | secondary" compact="true | false" loading="true | false">`:
  Standard action trigger. Enforces $\ge 48\times 48\text{px}$ touch target.
  ```html
...
- `<p-button-pure icon="..." underline="true | false" align-label="start | end">`:
  Minimal text button for inline triggers.
  ```html
...
- `<p-link-tile href="..." label="..." description="..." aspect-ratio="16:9">`: Visual media card with frosted overlay.
  ```html
...
- `<p-switch label="..." checked="true | false">`: Instant state toggle.
...
1. **NO RAW HEX OR ARBITRARY RGB COLORS (`TDS-RAW-COLOR`):**
   Never hardcode `#010205` or `#ffffff` in inline styles. Use CSS custom properties (`var(--p-color-primary)`, `var(--p-color-canvas)`) or design tokens.
