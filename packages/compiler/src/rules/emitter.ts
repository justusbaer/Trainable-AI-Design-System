...
5. **ACCESSIBLE NAMES ON ICON CONTROLS:** Every icon-only button must include an \`aria-label\` or visually hidden \`.sr-only\` text.
6. **FOCUS RINGS & OUTLINES:** Never strip focus outlines with \`outline-none\` without providing visible focus-visible ring styles (\`focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2\`).
7. **TYPESCALE LADDER:** Never use arbitrary font sizes/weights (\`text-[17px]\`, \`font-[550]\`, \`leading-[23px]\`). Use defined typescale tokens (\`text-title-medium\`, \`text-body-large\`).
8. **FLUID VIEWPORT CONTAINMENT:** Never set hardcoded wide container widths (\`w-[600px]\`, \`w-[1024px]\`) that cause horizontal overflow on mobile. Use fluid widths (\`w-full max-w-*\`) or responsive breakpoints.
9. **ON-COLOR PAIRING:** Always pair container backgrounds with their corresponding on-color tokens (\`bg-secondary-container\` with \`text-on-secondary-container\`, \`bg-error\` with \`text-on-error\`).
10. **STRICT SPEC & SKETCH FIDELITY (ZERO FEATURE HALLUCINATION):** Strictly adhere to what the user explicitly requested or drew in a sketch, mockup, or wireframe. NEVER invent, assume, or inject unrequested features, action buttons, widgets, or toggles (e.g. heating, flash & horn, tire pressure) to fill space.
11. **INTENTIONAL WHITESPACE & PLACEHOLDER MANDATE:** If an area is marked as whitespace (e.g. "Whitespace (for now)") or left unoccupied in a sketch/prompt, DO NOT invent features to occupy that space. Either ask the user proactively if they wish to add features there, OR render an explicit styled placeholder container indicating the space is intentionally left empty.
12. **SURFACE CONTAINMENT OVER OUTLINES & SHADOWS:** Never add arbitrary \`1px\` borders, outlines, or drop-shadows to cards, panels, or content surfaces. Modern flat container architectures establish visual hierarchy strictly through tonal surface contrast (\`surface-container-lowest\` through \`surface-container-highest\`). Only elements with an explicit \`outlined\` or \`elevated\` variant contract may receive borders or shadows.
13. **SUBPIXEL TEXT RENDERING & FONT-SMOOTHING CONTRACT:** Do NOT blindly apply \`-webkit-font-smoothing: antialiased;\` across stylesheets or root elements. Disabling subpixel rendering strips 100–150 weight units from letterforms on macOS/WebKit and makes typography appear frail. Always default to \`-webkit-font-smoothing: auto;\` unless explicitly mandated by the design system.
14. **INTERACTIVE PSEUDO-STATE FIDELITY:** Never guess or hallucinate hover/active styles (e.g. turning text link-blue or adding random underlines). Adhere strictly to the state contracts in \`components.json\`: navigation tabs/links use subtle color/weight shifts without underlines; editorial headlines underline without changing font color; action buttons use state-layer opacity overlays.
15. **ASSET VECTOR SANCTITY:** Brand marks, company logos, and emblems must ALWAYS be rendered using authentic vector SVGs from \`icons.json\` or inline SVGs. NEVER simulate or approximate brand logos using styled HTML text spans or colored character glyphs.
16. **AGENT SKILLS & AUDIT:** Refer to \`.agents/skills/tds-knowledge/SKILL.md\` for ambient guidelines, or run \`/tds-audit\` (\`npx tds audit\`) to generate an audit report in \`.tds/audits/<runId>/\`.
17. **EVALUATE YOUR CODE:** Run \`npx tds evaluate <file>\` to verify compliance before finalizing work.
`;
}
...
- **Accessible Names & Focus States:** Icon-only controls must have \`aria-label\` or \`.sr-only\` text. Never use \`outline-none\` without \`focus-visible:ring-*\`.
- **Typescale & Viewport Flow:** Never use arbitrary font sizing (\`text-[17px]\`, \`font-[550]\`) or fixed wide container widths (\`w-[600px]\`, \`w-[1024px]\`) that break mobile viewports.
- **Semantic On-Color Pairing:** Ensure container backgrounds pair with corresponding on-colors (e.g., \`bg-secondary-container\` with \`text-on-secondary-container\`, \`bg-error\` with \`text-on-error\`).
...
- Enforce sentence-case labels on buttons, tabs, and headers.
- **Accessible Names & Focus States:** Every icon control must have \`aria-label\` or \`.sr-only\` text. Do not strip focus outlines (\`outline-none\`) without providing \`focus-visible:ring-*\`.
- **Typescale & Viewport Flow:** Never use arbitrary font sizing (\`text-[17px]\`, \`font-[550]\`) or fixed container widths (\`w-[600px]\`, \`w-[1024px]\`) that trigger mobile horizontal scrolling.
- **Semantic On-Color Pairing:** Ensure container backgrounds pair with corresponding on-colors (e.g., \`bg-secondary-container\` with \`text-on-secondary-container\`, \`bg-error\` with \`text-on-error\`).
